...
emx_inval_custom() {
  global
  InputBox, emx_inval_custom, Invalidate Emacs Key Bindings, Enter keys to invalidate with comma`n(e.g. "b`,d"),,300,150
}
+<^i::emx_inval_custom()
...
is_emx_mode(chr) {
  ...
  if chr not in %emx_inval_custom%
  {
    Return 1
  } else {
    Return 0
